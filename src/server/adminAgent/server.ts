...
export function startAdminDashboardServer(): http.Server {
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (url.pathname === '/' || url.pathname === '/index.html') return send(res, ADMIN_HTML, '', 'text/html; charset=utf-8');
