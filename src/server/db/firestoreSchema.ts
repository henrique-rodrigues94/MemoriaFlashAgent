...
export interface BankCard { id: string; front: string; back: string; explanation: string; topic: string; subtopic?: string; difficulty: string; contentHash?: string; version?: number; qualityScore?: number; accuracyScore?: number; relevanceScore?: number; groundingScore?: number; sourceHash?: string; sourceSection?: string; sourcePage?: number; sourceExcerpt?: string; agentVersion?: string; promptVersion?: string; model?: string; }
...
export interface CardFeedbackDoc { cardId: string; bucketId: string; subject: string; topic: string; subtopic?: string; level: EducationLevel; cardType: CardContentType; difficulty: string; rating: FeedbackRating; reason?: FeedbackReason; comment?: string; createdAt: string; status?: string; processedAt?: string; failureReason?: string; correctionAttempts?: number; }
...
export type ContentRequestStatus = 'pending' | 'processing' | 'extracting' | 'analyzing' | 'generating' | 'validating' | 'publishing' | 'completed' | 'failed' | 'building' | 'error';
...
export interface ContentRequestDoc { subject?: string; requestedSubject?: string; normalizedSubject?: string; subjectId: string; educationLevel?: EducationLevel; status: ContentRequestStatus; priority: number; requestCount: number; attempts: number; createdAt: string; requestedAt?: string; updatedAt: string; requestedBy?: string; lastError?: string; error?: string; duplicateOf?: string; recoveredAt?: string; nextRetryAt?: string; lastErrorAt?: string; stage?: 'queued' | 'extracting' | 'analyzing' | 'generating' | 'validating' | 'publishing' | 'completed' | 'failed'; source?: ContentRequestSourceMeta; progress?: { levels: number; curriculaReady: number; leavesDiscovered: number; cardsGenerated: number; cardsApproved?: number; cardsRejected?: number; qualityScore?: number }; }
export interface AgentMetricDoc { date: string; runs: number; aiCalls: number; geminiCalls: number; deepseekCalls: number; openaiCalls: number; cardsGenerated: number; cardsApproved?: number; cardsRejected?: number; tokensInput: number; tokensOutput: number; cacheHits: number; cacheMisses: number; estimatedAiCostUsd: number; duplicates: number; errors: number; updatedAt: string; }
