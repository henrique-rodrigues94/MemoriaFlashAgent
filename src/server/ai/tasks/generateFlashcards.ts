...
export async function generateFlashcardsTask(args:{prompt:string;count?:number;language?:string;difficulty?:string;selectedTopics?:string[];educationLevel?:EducationLevel;sourceType?:GenerationSourceType;existingFronts?:string[];cardContentType?:string;sourceContext?:string}){
  const {prompt,count=6,language='pt',difficulty='medium',selectedTopics=[],educationLevel='medio',sourceType='subject',existingFronts=[],cardContentType='definition',sourceContext=''}=args;
  ...
  const cardTypeInstruction=CARD_TYPE_PROMPTS[cardContentType]??CARD_TYPE_PROMPTS.definition;const useBank=sourceType==='subject';const existingFrontsSet=new Set<string>(existingFronts);const normalizedTopics=sanitizeTopics(selectedTopics);
