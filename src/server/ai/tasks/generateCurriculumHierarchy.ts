...
function schema() { return { type: Type.OBJECT, properties: { categories: { type: Type.ARRAY, minItems: 4, maxItems: 12, items: { type: Type.OBJECT, properties: { category: { type: Type.STRING }, topics: { type: Type.ARRAY, minItems: 3, maxItems: 10, items: { type: Type.OBJECT, properties: { topic: { type: Type.STRING }, subtopics: { type: Type.ARRAY, minItems: 3, maxItems: 8, items: { type: Type.STRING } } }, required: ['topic', 'subtopics'] } } }, required: ['category', 'topics'] } } }, required: ['categories'] }; }
function compactSource(sourceContext: string): string { const source = normalizeText(sourceContext); if (source.length <= 30000) return source; const head = source.slice(0, 10000); const middleStart = Math.max(10000, Math.floor(source.length / 2) - 5000); const middle = source.slice(middleStart, middleStart + 10000); const tail = source.slice(-10000); return `${head}\n\n[...trecho central... ]\n\n${middle}\n\n[...final do documento...]\n\n${tail}`; }
...
export async function generateCurriculumHierarchyTask(args: { subject: string; educationLevel: EducationLevel; language?: string; sourceContext?: string; }): Promise<{ categories: CurriculumCategory[]; providerUsed: string; cacheHit?: boolean; enriched?: boolean }> {
  ...
  // Mantemos uma representação plana adicional para métricas/compatibilidade
  // do Firestore. Os dados exibidos pelo app continuam vindo de categories.
  const topicTree = categories.flatMap(category => category.topics.map(topic => ({ topic, subtopics: category.subtopics[topic] ?? [topic] })));
  await saveCurriculum(subject, educationLevel, categories as any, providerUsed, topicTree);
