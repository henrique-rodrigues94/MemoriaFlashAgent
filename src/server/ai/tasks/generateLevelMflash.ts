...
async function generateExactly(subject: string, level: EducationLevel, topic: string, subtopic: string, count: number) {
  ...
  while (cards.length < count && attempts < 4) {
    ...
    for (const card of result.cards) {
      ...
      const key = front.toLocaleLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
      if (existingFronts.some(x => x.toLocaleLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '') === key)) continue;
...
export async function generateLevelMflash(args: { subject: string; educationLevel: EducationLevel; cardsPerSubtopic?: number; outputDir?: string }): Promise<{ outputPath: string; package: MflashPackage; totalCards: number; totalSubtopics: number }> {
  ...
  // FASE 1 — somente currículo: carregamos e congelamos a grade antes de qualquer geração de cards.
  ...
  const uniqueRows = Array.from(new Map(rows.map(row => [`${row.category}::${row.topic}::${row.subtopic}`.toLocaleLowerCase(), row])).values());
  console.log(`[level-mflash] FASE 1 concluída: ${uniqueRows.length} subtópicos válidos encontrados na grade ${subject}/${level}.`);
  console.log(`[level-mflash] FASE 2 iniciada: ${cardsPerSubtopic} cards independentes por subtópico.`);

  // FASE 2 — geração isolada: uma chamada/lote por subtópico, com tentativas somente para completar o saldo.
  ...
  for (let index = 0; index < uniqueRows.length; index++) {
    ...
    console.log(`[level-mflash] OK: ${cards.length}/${cardsPerSubtopic} cards — ${row.subtopic}`);
  }
  ...
  const packageData: MflashPackage = {
    manifest: {
      ...
      language: 'pt-BR',
      levels: [level],
      statistics: { levels: 1, subjects: 1, curricula: 1, topics: levelPackage.subjects[0].curricula[0].topics.length, subtopics: uniqueRows.length, cards: totalCards, cardsPerSubtopic },
      generator: { name: 'MemoriaFlashAgent', mode: 'level', modelPolicy: 'configured-provider', target: `${subject}_${level}`, generationStrategy: 'curriculum-first-subtopic-batches' },
    },
    levels: [levelPackage],
  };
  ...
  return { outputPath, package: packageData, totalCards, totalSubtopics: uniqueRows.length };
}
