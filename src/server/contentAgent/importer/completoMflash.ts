...
export interface MflashCardInput {
  id?: string;
  question?: string;
  front?: string;
  answer?: string;
...
function canonicalCard(card: MflashCardInput): { front: string; back: string; explanation: string; curiosity: string; difficulty: string } {
  return {
    front: text(card.front || card.question),
    back: text(card.back || card.answer),
    explanation: text(card.explanation),
    curiosity: text(card.curiosity),
    difficulty: text(card.difficulty || 'medium').toLowerCase(),
  };
}
function normalizeDifficulty(value: string): string {
  ...
  if (['specialist', 'especialista'].includes(v)) return 'specialist';
  return 'medium';
}
function qualityScore(card: ReturnType<typeof canonicalCard>, hierarchyOk: boolean): number {
  let score = 0;
  if (card.front.length >= 10) score += 20;
  if (card.back.length >= 15) score += 20;
  if (card.explanation.length >= 15) score += 20;
  if (card.curiosity.length >= 10) score += 10;
  if (hierarchyOk) score += 15;
  if (card.front.length <= 600 && card.back.length <= 3000) score += 15;
  return score;
}
...
export function validateMflashPackage(pkg: unknown): { package?: MflashPackage; issues: ValidationIssue[] } {
  ...
  if (manifest?.package !== 'completo') issues.push({ severity: 'error', code: 'PACKAGE_INVALID', path: 'manifest.package', message: 'O pacote oficial deve ser completo.' });
  ...
  if (!Array.isArray(manifest?.levels) || OFFICIAL_LEVELS.some(level => !manifest.levels.includes(level))) issues.push({ severity: 'error', code: 'MANIFEST_LEVELS_INCOMPLETE', path: 'manifest.levels', message: 'O manifesto precisa declarar os cinco níveis oficiais.' });
...
export async function publishStagedImport(jobId: string): Promise<ImportPlan> {
  ...
  for (const level of validated.package.levels) {
    for (const subject of level.subjects) {
      ...
      if (!arr.some(x => x.level === level.id)) arr.push({ level: level.id, label: levelLabel(level.id), icon: 'book', reason: 'Importado do pacote editorial completo.mflash', priority: 5 });
