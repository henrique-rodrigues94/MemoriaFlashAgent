...
export function validateProductionEnvironment(env: NodeJS.ProcessEnv = process.env): ProductionCheck[] {
  ...
  checks.push({ name: 'ai-provider', ok: hasGemini || hasDeepSeek || hasOpenAI, detail: hasGemini ? 'Gemini configurado' : hasDeepSeek ? 'DeepSeek configurado' : hasOpenAI ? 'OpenAI configurado' : 'nenhum provedor configurado' });
  ...
  checks.push({ name: 'ai-call-limit', ok: maxAiCalls <= 500, detail: String(maxAiCalls) });
