...
export async function runContentAgent():Promise<AgentRunSummary>{
  ...
  try{
    ...
    if(mode==='update_requested'||mode==='automatic'){tracker.log({action:'[mode:update_requested] processando somente matérias/assuntos solicitados por usuários'});await processContentRequestsJob(tracker);}
    if(mode==='discover_new'){tracker.log({action:'[mode:discover_new] pesquisando candidatos de conteúdo antes de gerar qualquer card'});await discoverNewContentJob(tracker);}
